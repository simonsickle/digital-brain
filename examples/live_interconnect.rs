...
fn main() -> digital_brain::Result<()> {
    ...
    brain.believe("Emotional memories persist longer", BeliefCategory::WorldModel, 0.85);
    ...
    let stimuli = vec![
        ...
        ("Positive", "Amazing! I just made a breakthrough in memory research!"),
        ("Negative", "Warning: critical error detected in the system."),
        ("Surprising", "Unexpected: the prediction was completely wrong!"),
        ("Self-referential", "I am thinking about my own thoughts right now."),
    ];

    for (category, stimulus) in stimuli {
        println!("┌─ STIMULUS: {} ──────────────────────────────────────────────────", category);
        ...
        let valence_label = if valence > 0.3 { "POSITIVE 😊" } 
        ...
        println!("│ │ Significant: {}", if result.emotion.is_significant { "YES ⚡" } else { "no" });
        println!("│ └────────────────────────────────────────────────────────────────┘");
        
        // Show consciousness access
        let conscious_icon = if result.reached_consciousness { "⭐ YES" } else { "no" };
        println!("│");
        ...
        if let Some(ref mem) = result.memory {
            let decay = if valence.abs() > 0.5 { "SLOW (emotional)" } else { "normal" };
    ...
    println!("╠═══════════════════════════════════════════════════════════════════════╣");
    println!("║  Processing cycles:    {:>5}                                         ║", stats.cycles);
    println!("║  Memories stored:      {:>5}                                         ║", stats.memories);
    println!("║  Conscious items:      {:>5}                                         ║", stats.conscious_items);
    println!("║  Working memory items: {:>5}                                         ║", stats.working_memory_items);
    println!("║  Active beliefs:       {:>5}                                         ║", stats.beliefs);
    println!("║  Emotional state:     {:>+.2}                                          ║", stats.emotional_state);
    println!("║  Current learning rate: {:.3}                                         ║", stats.learning_rate);
    println!("║  Signals processed:   {:>5}                                          ║", stats.signals_processed);
    println!("║  Signals passed:      {:>5}                                          ║", stats.signals_passed);
    println!("║  Signals filtered:    {:>5}                                          ║", stats.signals_filtered);
    println!("╚═══════════════════════════════════════════════════════════════════════╝\n");
    ...
    println!("│ │ Memories consolidated: {} (moved to long-term storage)", sleep_report.memories_consolidated);
    println!("│ │ Memories forgotten: {} (decayed below threshold)", sleep_report.memories_forgotten);
    ...
    println!("╠═══════════════════════════════════════════════════════════════════════╣");
    println!("║  Memories remaining:   {:>5} (emotional ones persisted)              ║", final_stats.memories);
    println!("║  Emotional state:     {:>+.2} (more neutral after sleep)              ║", final_stats.emotional_state);
    println!("╚═══════════════════════════════════════════════════════════════════════╝\n");
