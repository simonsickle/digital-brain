...
fn test_valence_weighted_decay() {
    ...
    let emotional = BrainSignal::new("test", SignalType::Memory, "emotional event")
        .with_valence(0.9);
    let neutral = BrainSignal::new("test", SignalType::Memory, "neutral event")
        .with_valence(0.0);
    ...
    assert!(emotional_mem.strength > neutral_mem.strength,
...
fn test_access_strengthening() {
    ...
    assert!(after_access > before_access,
        "Strength should increase after access: {} -> {}", before_access, after_access);
}
...
fn test_statistics() {
    ...
    for v in valences {
        let signal = BrainSignal::new("test", SignalType::Memory, "mem")
            .with_valence(v);
        store.encode(&signal).unwrap();
    }
